...
blocks.forEach(block => {
  ...
  block.addEventListener('mousemove', () => {
    if (draggedBlock) dragMoved = true;
  });
  ...
  block.addEventListener('pointerup', (e) => {
    ...
    const from = selectedBlock.dataset.id;
    const to   = id;
    if (from === 'input' && to === 'encoder') {
      addConnection('input', 'output', 'encoder', 'input');
    }
    if (from === 'encoder' && to === 'oscilloscope') {
      addConnection('encoder', 'output', 'oscilloscope', 'input');
    }
    if (from === 'encoder' && to === 'psd') {
      addConnection('encoder', 'output', 'psd', 'input');
    }
...
function updateConnections() {
    ...
    const disconnectThreshold = Infinity; 
...
function addConnection(startBlock, startType, endBlock, endType) {

  const exists = connections.some(c =>
    c.start.block === startBlock &&
    c.end.block === endBlock
  );
  if (exists) return;
  connections.push({
    start: { block: startBlock, type: startType },
    end:   { block: endBlock,   type: endType }
  });
  updateConnections();
}
...
const binaryInputField = document.getElementById("binaryInput");

binaryInputField.addEventListener("input", function () {
...
hintToggle.addEventListener("click", () => {
    hintPanel.classList.toggle("fade-hidden");
});
// Normal encoding example
function setNormalInput() {
    const input = document.querySelector("#binaryInput");
    if (input) {
        input.value = "10110100";
    }
}
// HDB3 / B8ZS example (long zero sequence)
function setSpecialInput() {
    const input = document.querySelector("#binaryInput");
    if (input) {
        input.value = "1000000000010000";
    }
}
// AUTO CONNECT BLOCKS
